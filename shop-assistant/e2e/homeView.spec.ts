...
test('check product container', async ({ page }) => {
  ...
  await expect(
    page.locator(
      '#app > div > main > div > section > section:nth-child(1) > div.productName-container > button:nth-child(2)'
    )
  ).toBeVisible()
  await expect(
    page.locator(
      '#app > div > main > div > section > section:nth-child(1) > div.productName-container > button:nth-child(2) > svg.fa-angle-up'
    )
  ).toBeVisible()
  await expect(
    page.locator(
      '#app > div > main > div > section > section:nth-child(1) > div.productName-container > button.custom-button.sort-down > svg'
    )
  ).toBeVisible()
  await expect(
    page.locator(
  ...
  ).toHaveAttribute('data-prefix', 'far')
})
